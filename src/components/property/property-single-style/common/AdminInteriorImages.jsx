...
const AdminInteriorImages = ({ interior }) => {
          ...
          <ul className="mb20 ps-0 d-flex flex-wrap">
            {images.map((image, index) => (
              <li className="list-inline-item mb15 me15" key={index}>
                <Item
                  original={adminBaseUrl + image}
                  thumbnail={adminBaseUrl + image}
                  width={1100}
                  height={700}
                >
                  {({ ref, open }) => (
      ...
      <div className="col-md-12">
        <div className="position-relative bdrb1 pt30 pb20">
          <div className="text-muted">
            ...
            {images.length} property image{images.length !== 1 ? "s" : ""}{" "}
            available
          </div>
        </div>
      </div>
